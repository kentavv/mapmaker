...
/*** Prompting Input Editor added by Mark J Daly, mjdaly@athena.mit.edu ****/

char *gnu_edit_this(prompt_str,start_str)
char *prompt_str, *start_str;
{
    static rl_prep_terminal (), rl_deprep_terminal ();
    char *readline_internal ();
    char *newstr;
    ...
    rl_initialize();
    rl_prep_terminal();
    strcpy(rl_line_buffer,start_str);
    rl_end= strlen(start_str);
    strcpy(rl_prompt,prompt_str);
    newstr= readline_internal();  /* maybe null */
    rl_deprep_terminal();
