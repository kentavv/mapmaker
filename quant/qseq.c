...
{ 
    ...
    array(seqtoken,TOKLEN+1,char); seqtoken_ptr=seqtoken;
...
char *str;
{
    ...
    run {
	do {
	    p=int_compiler(&ptr); if (p==NULL) send(CRASH);
	    enumerate_possibilities(p);
	    if (first==NULL) first=last=p; else { last->next=p; last=p; }
	} while (!nullstr(ptr));
...
char **str; 
{
	...
	run {
	  ...
	  me->left= NULL; me->right= NULL;
	  me->fix_pos= NULL; me->contig=NULL;
	  me->cont_var=NULL; me->count=NULL;

	} when_aborting {
	    unsingle(me,QTL_SEQUENCE);
	    unsingle(p,QTL_SEQ_OPTION);
	    while (first!=NULL)
	      { p=first->next; unsingle(first,QTL_SEQ_OPTION); first=p; }
	    relay_messages;
	}
        return(me);
}
...
QTL_SEQ_OPTION *opt; /* may be side-effected */
{
	...
	opt->isa.interval.fix_pos=pos; return(TRUE);
    } else return(FALSE);
}
...
QTL_SEQ_OPTION *opt; /* may be side-effected (turned into a RANGE) */
{
	...
	opt->isa.range.to=num; opt->type=RANGE; return(TRUE);
    } else return(FALSE);
}
...
GENETICS *g;
{
    ...
    else if (wizard_mode && nmatches(seqtoken,"f3dominant",3))
      { INTERX_TYPE_(F3DOMINANT) }
    else if (wizard_mode && nmatches(seqtoken,"f3recessive",3)) 
      { INTERX_TYPE_(F3RECESSIVE) }
	...
	if (raw.data_type==INTERCROSS) {
	    ...
	    g->interx_type=FIXED; g->a=a; g->b=d; 
...
QTL_SEQUENCE *p;
{
    ...
    i=0; q=p->option; if (q==NULL) send(CRASH);
...
int *pnum_intervals, *pcont_vars, *pnum_orders, *pwiggle_ints;
{
    ...
    num_orders=1; num_intervals=0; wiggle_ints=0; cont_vars=0;
    do { /* for all interval specs, which may be repeated */
	num_models=1; p->test=0; 
	...
	if (wiggle && p->next==NULL) { /* rightmost interval in a wiggle */
	    ...
	    if (p->isa_continuous_var) for (i=0; i<p->num_options; i++) 
	...
	if (p->count[i]<contig_end) { /* can permute, usually... */
	    ...
	    *perm+=i; 
		...
		if (i<0 && wiggle && p->next==NULL) {
		    p->count[0]=0; p->pos_cm= 0.0;
		    p->interval_cm= 
		      haldane_cm(map_length(p->left[0],p->right[0]));
		} else for (j=i+1; j<p->repeat; j++) { 
...
char *str;
{
	...
	while (stoken(&str,sREQUIRED,tok)) {
		...
		if (get_named_entry(tok,&dummy,&expansion,
		    context[active_context]->named_sequences,&err))
		  strcat(new_str,expansion); else strcat(new_str,tok);
		strcat(new_str," ");
	    }
	}
	    self_delimiting=ptr_to("");
