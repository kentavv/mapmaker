...
/*    mkcommand("prepare data",             "",   prep_data,       CMD); */
    ...
    min_trait_val = -VERY_BIG;
    max_trait_val = VERY_BIG;
}
...
translate(void) {
    ...
    for (i = 0; i < raw.n_loci; i++) {
        if ((printed++) % 5 == 0) nl();
        sprintf(ps, "%3d %-10s ", raw.original_locus[i], raw.locus_name[i]);
        pr();
    }
    nl();
...
show_best(void) {
    ...
    if (contig < 0) {
        sprintf(ps, SAVED_COMP_BEST, maybe_s(op->num_contigs), compare + 1, 1);
        pr();
        if (op->num_contigs > 1) {
            sprintf(ps, SAVED_COMP_COMPS, compare + 1, op->num_contigs);
            pr();
        }
        sprintf(ps, SAVED_COMP_TRAIT, op->trait + 1, raw.trait_name[op->trait]);
        pr();
        print_old_seq(op->seq_string);
        sprintf(ps, SAVED_COMP_THRESH, threshold, falloff);
        pr();
        for (i = 0; i < op->num_contigs; i++) {
            nl();
            sprintf(ps, SAVED_COMP_NUM, compare + 1, i + 1);
            pr();
            print_best_saved_maps(compare, i, threshold, falloff);
        }

    } else {
        sprintf(ps, SAVED_COMP_BEST, "", compare + 1, contig + 1);
        pr();
        sprintf(ps, SAVED_COMP_TRAIT, op->trait + 1, raw.trait_name[op->trait]);
        pr();
        print_old_seq(op->seq_string);
        ...
        pr();
        nl();
        print_best_saved_maps(compare, contig, threshold, falloff);
    }
}
...
show_compare(void) {
    ...
    if (contig < 0) {
        sprintf(ps, SAVED_COMP_WHICH, maybe_s(op->num_contigs), compare + 1, 1);
        pr();
        if (op->num_contigs > 1) {
            sprintf(ps, SAVED_COMP_COMPS, compare + 1, op->num_contigs);
            pr();
        ...
        sprintf(ps, SAVED_COMP_WHICH, "", compare + 1, contig + 1);
        pr();
        sprintf(ps, SAVED_COMP_TRAIT, op->trait + 1, raw.trait_name[op->trait]);
        pr();
        print_old_seq(op->seq_string);
        nl();
        print_saved_maps(compare, contig);
    }
}
...
wiggle(void) {
    ...
    if (print_scans) {
        ...
        print_trait(2);
        print_seq();
        sprintf(ps, THRESHOLD_AND_SCALE, threshold, scale);
        pr();
    }
    run {
...
list_wiggles(void) {
    ...
    sprintf(ps, SAVED_WIGGLE_WIG, maybe_s(op->num_orders), wiggle + 1, 1);
    pr();
    if (op->num_orders > 1) {
        sprintf(ps, SAVED_WIGGLE_WIGS, wiggle + 1, op->num_orders);
        pr();
    }
    sprintf(ps, SAVED_WIGGLE_TRAIT, op->trait + 1, raw.trait_name[op->trait]);
    pr();
    print_old_seq(op->seq_string);
    nl();
...
show_wiggle(void) {
    ...
    if (order < 0) {
        ...
        sprintf(ps, SAVED_WIGGLE_WIG, "", wiggle + 1, order + 1);
        pr();
        sprintf(ps, SAVED_WIGGLE_TRAIT, op->trait + 1, raw.trait_name[op->trait]);
        pr();
        print_old_seq(op->seq_string);
...
show_peaks(void) {
    ...
    run {
            ...
            sprintf(ps, PEAKS_TITLE, wiggle + 1, order + 1, wiggles[wiggle]->trait + 1,
                    raw.trait_name[wiggles[wiggle]->trait]);
            pr();
            ...
            sprintf(ps, PEAKS_PARAMS, threshold, falloff);
            pr();
            nl();
...
make_trait(void) {
    ...
    if (!valid_new_trait_name(name_to_check, errmsg)) {
        sprintf(ps, TRAIT_BADNAME, name_to_check, errmsg);
        error(ps);
    }
    ...
    run {
            ...
            nl();
            sprintf(ps, TRAIT_MADE, trait_index + 1, name_to_check);
            pr();
            nl();
            ...
            nl();
            print_rhisto(normal_array, adjusted_array_size);
        } on_exit {
...
predict(void) {
    ...
    if (!valid_new_trait_name(name, errmsg)) {
        sprintf(ps, TRAIT_BADNAME, name, errmsg);
        error(ps);
    }
    ...
    nl();
    sprintf(ps, TRAIT_MADE, trait_index + 1, name);
    pr();
    nl();
    if (!trait_redone) raw.n_traits++;
...
forget_trait(void) {
    ...
    sprintf(ps, "Deleting trait %d (%s)", t + 1, raw.trait_name[t]);
    pr();
    if (!nullstr(raw.trait_eqn[t])) {
        sprintf(ps, "='%s'", raw.trait_eqn[t]);
        pr();
    ...
    raw.trait_name[t][0] = '\0';
    raw.trait_eqn[t][0] = '\0';
    if (t == raw.n_traits - 1) raw.n_traits--;
    print("Trait Deleted.\n");
    if (t == trait) {
        trait = -1;
        print("The current trait is now not set!\n");
    }
...
forget_scan(void) {
    ...
    if (print_names)
        ...
        sprintf(ps, WIG_LIST_NUMS, t + 1, (op->num_orders == 1 ? "1" : "x"),
...
//    int num_of_file=0;
    ...
    if (nullstr(args)) {
        if (!data_loaded()) print(NO_LOADED_DATA);
        else {
            ...
            sprintf(ps, LOADED_DATA, raw.file, tfile, "", raw.n_indivs,
                    ...
                    raw.n_loci, raw.n_traits, maybe_s(raw.n_traits));
            pr();
        }

    } else { /* !nullstr */

        nstoken(&args, sREQUIRED, dfile, PATH_LENGTH);
        tfile[0] = '\0';
        nomore_args(num_args);
        run {
                ...
                sprintf(ps, LOADED_DATA, dfile, tfile, "", raw.n_indivs,
                        (raw.data_type == BACKCROSS ? "backcross" : "intercross"),
                        raw.n_loci - dum_loc, raw.n_traits, maybe_s(raw.n_traits));
                pr();
...
load_qtl_files(void) {
    ...
    run {
            ...
            when CANTOPEN:
                ...
                msg = 0;
                break; /* No .qtls file - WHY IS THIS MSG=0 NEEDED??? */
            default:
                sprintf(ps, QTL_LD_ERROR1, name);
                pr();
                print(QTL_LD_ERROR2);
                print(QTL_LD_ERROR3);
                close_file(fp);
...
tester(void) {
    ...
    for (left = 0; left < 4; left++)
        for (right = 0; right < 4; right++) {
            ...
            x = y = z = 0.0;
            raw.f3 = TRUE;
            c = transition_prob(INTERCROSS, left, right, theta);
