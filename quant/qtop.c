...
void ps_MATINV(char *a) {
    sprintf(ps, "diff= %lf", MATINV_diff);
}

void ps_BADDATA(char *a) {
    sprintf(ps, "line=\"%s\"\nerror=%s",
...
void top_init(void) {
    ...
    NOPARSE_err = 0;
    BADSEQ_errpos = 0;
    BADSEQ_errmsg = BADTRAIT_errmsg = NULL;

    array(BADDATA_error, MAXLINE + 1, char);
    map = NULL;
    map_data = NULL;
    trait = -1;
    array(trait_string, MAXLINE, char);
...
QTL_MAP *alloc_qtl_map(int num_intervals, int num_cont_vars) {
    ...
    run {
            ...
            map->interval_len = NULL;
            map->qtl_pos = map->qtl_weight = NULL;
            map->fix_pos = NULL;
            map->constraint = (GENETICS *) NULL;

            map->max_continuous_vars = 0;
            map->cont_var = NULL;
            map->cont_var_weight = NULL;
            map->fix_cont_var_weight = NULL;
...
int add_interval(QTL_MAP *map, int left, int right, /* loci */ real fix_pos, GENETICS *genetics) {
    int i;
...
void mapcpy(QTL_MAP *to, QTL_MAP *from) {
    /* strcpy()-ish backasswards notation */
    int i;
...
void qtl_ready(int data_type, int need_seq, bool need_trait, bool will_call_qctm) {
/* Side-effects globals num_intervals, num_orders, num_ints_to_wiggle,
   free_genetics, and allocate map and map_data, if needed. */
    ...
    if (data_type == INTERCROSS && raw.data_type != INTERCROSS) {
        sprintf(ps, eNEED_INTERX, com);
        error(ps);
    }
    if (data_type == BACKCROSS && raw.data_type != BACKCROSS) {
        sprintf(ps, eNEED_BACKX, com);
        error(ps);
    }
    ...
    if (need_seq != NOSEQ) {
        ...
        if (!reset_state(ints, (need_seq == WIGSEQ), &num_intervals,
            ...
            sprintf(ps, eWRONG_SEQ, com);
            error(ps);
        }
        if (need_seq == ONEINT && num_intervals != 1) {
            sprintf(ps, eONE_INT, com);
            error(ps);
        } /* FROB */
    ...
    if (will_call_qctm) {
        ...
        if (map_data == NULL)
            map_data = alloc_data(max_intervals,
                                  max_continuous_vars);
    }
}
...
bool valid_locus_str(char *str,     /* assume this is a despaced/lowercased/filtered token */
    ...
    if (nullstr(str)) {
        ...
        if (name[0] == '*') {
            name++;
            if (nullstr(name)) {
                strcpy(errmsg, eNOLOCUS);
                return (FALSE);
            }
        }
        if (!valid_name(name)) {
            strcpy(errmsg, "illegal name");
            return (FALSE);
        }
        if (len(name) > NAME_LEN) {
            sprintf(errmsg, eNAMETOOLONG, name, NAME_LEN);
            return (FALSE);
        }
...
bool valid_trait_str(char *str,   /* assume this is a despaced/lowercased/filtered token */
                     int *num,     /* side-effect with the number if TRUE */
                     char *errmsg /* side-effect with a message if FALSE */) {
    /* return TRUE or FALSE */
    int match, exact;
    ...
    if (nullstr(str)) {
        ...
        if (!valid_trait_num(*num)) {
            ...
            sprintf(errmsg, eTRAITKILLED, *num + 1);
            return (FALSE);
        } else if (!nullstr(str)) { /* check for only one token */
            strcpy(errmsg, eNTRAITTOKS);
            return (FALSE);
        } else return (TRUE);
...
bool valid_new_trait_name(char *str,     /* assume this is a despaced/lowercased/filtered token */
                          char *errmsg  /* side-effect with a message if FALSE */) {
    /* return TRUE or FALSE */
    int match, exact;
    char *name = get_temp_string();
    ...
    if (nullstr(str)) {
        ...
        } else if (!nullstr(str)) /* check for only one token */
        {
            ...
            sprintf(errmsg, eNAMEINUSE, name);
            return (FALSE);
        }
        return (TRUE);
