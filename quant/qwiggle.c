...
static WIGGLE_PEAK *
peak_finder(int *interval, int *point, bool *off_end, bool get_peak_maps /* if TRUE, fill in peak->map MAY REQUIRE CALCULATION! */,
            WIGGLE_INTERVAL **wig, int n_intervals,
...
void store_wiggle_interval(int wiggle_num, QTL_MAP *map, bool new_left_order, bool contig, real cm_step) {
    ...
    if (wiggle_num >= max_wiggles || wiggles[wiggle_num] == NULL ||
        (op = wiggles[wiggle_num])->data == NULL || map == NULL)
        send(CRASH);
    ...
    if (new_left_order) {
        ...
        j = (op->wiggle_interval = 0);
        i = (op->order += 1);
    } else {
        if (op->wiggle_interval >= op->num_wiggled_intervals - 1) send(CRASH);
        i = op->order;
        j = (op->wiggle_interval += 1);
    }

    k = map->num_intervals - 1; /* the rightmost interval */
    data = op->data[i][j];
    if (data == NULL) send(CRASH);
    data->point = NULL;
    data->map = NULL;
    run {
            data->contig = contig;
...
void store_compare_map(int compare_num, QTL_MAP *map, bool contig) {
    ...
    if (compare_num >= max_compares || compares[compare_num] == NULL ||
        (op = compares[compare_num])->data == NULL || map == NULL)
        send(CRASH);

    if (op->order < 0 || !contig) op->num_contigs++;
    i = (op->order += 1);
    if (i >= op->num_orders) send(CRASH);
    data = op->data[i];
    if (data == NULL) send(CRASH);
    data->map = NULL;
...
WIGGLE_PEAK *peak_finder(int *interval, int *point, bool *off_end, bool get_peak_maps /* if TRUE, fill in peak->map MAY REQUIRE CALCULATION! */,
                         WIGGLE_INTERVAL **wig, int n_intervals,
                         real threshold, real falloff) {
    ...
    run {
            *off_end = TRUE;
            ...
            right_i = peak_i = *interval;
            right_j = peak_j = *point;
...
get_wiggle_nums(char *str, int *wiggle, int *order) {
    ...
    if ((i = strfinder(str, '.')) > 0) {
        str[i++] = '\0';
        if (sscanf(str, "%d", wiggle) < 1) usage_error(num_args);
        order_set = TRUE;
        if (sscanf(&str[i], "%d", order) < 1)
        ...
        *order = -1;
        if (sscanf(str, "%d", wiggle) < 1) usage_error(num_args);
        order_set = FALSE;
        if ((--*wiggle) < 0) usage_error(num_args);
    }
    ...
    if (order_set)
        if (*order >= wiggles[*wiggle]->num_orders) {
            if (wiggles[*wiggle]->num_orders == 1)
                ...
                sprintf(ps, ORDER_NUM, *wiggle + 1, *order + 1, *wiggle + 1, *wiggle + 1,
...
get_compare_nums(char *str, int *compare, int *contig) {
    ...
    if ((i = strfinder(str, '.')) > 0) {
        str[i++] = '\0';
        if (sscanf(str, "%d", compare) < 1) usage_error(num_args);
        contig_set = TRUE;
        ...
        *contig = -1;
        if (sscanf(str, "%d", compare) < 1) usage_error(num_args);
        contig_set = FALSE;
        if ((--*compare) < 0) usage_error(num_args);
    }
...
void load_interval(FILE *fp, WIGGLE_INTERVAL *interval) {
    ...
    run {
            ...
            interval->num_points = num_points;
            interval->cm_increment = inc;
            interval->contig = i1;
            interval->max_like_map = i2;
            interval->in_qtls_struct = i3;
            interval->best_point = i4;

            /* load map */
            interval->map = load_qtl_map(fp);

            /* load other data points */
            if (interval->num_points > 0) parray(interval->point, interval->num_points, WIGGLE_POINT);
...
QTL_MAP *load_qtl_map(FILE *fp) {
    ...
    map->trait = i1;
    map->num_intervals = i2;
    map->max_intervals = i3;
    map->num_continuous_vars = i4;
    map->max_continuous_vars = i5;
    ...
    for (i = 0; i < map->num_intervals; i++) {
        ...
        map->left[i] = i1;
        map->right[i] = i2;
        map->interval_len[i] = r1;
        map->qtl_pos[i] = r2;
        map->qtl_weight[i] = r3;
        map->qtl_dominance[i] = r4;
        map->fix_pos[i] = r5;
        ...
        map->constraint[i].interx_type = i1;
        map->constraint[i].a = r2;
        map->constraint[i].b = r3;
        map->constraint[i].c = r4;
    }
    ...
    map->mu = r1;
    map->sigma_sq = r2;
    map->var_explained = r3;
    map->chi_sq = r4;
    map->null_mu = r5;
    map->null_sigma_sq = r6;
    map->null_log_like = r7;
    map->log_like = r8;
    map->no_data_like = r9;
    map->abs_log_like = r10;
