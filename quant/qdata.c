...
void prepare_data(QTL_MAP *map, DATA *data /* side-effected */) {
    ...
    if (indiv == 0) { error("trait(s) are all missing data for all individuals."); }
...
void alloc_qctm_globals(void) {
    ...
    if (max_intervals > MAX_INTERVALS ||
        max_continuous_vars > MAX_CONTINUOUS_VARS)
        send(CRASH);
...
real model_prediction(QTL_MAP *map, int indiv) {
    ...
    for (j = 0; j < map->num_intervals; j++) { /* fill in prob[] */
        ...
        for_locus_genotypes(raw.data_type, qtl) qtl_geno_prob[qtl] = 0.0;
