...
void save_traitfile(FILE *fp) {
    ...
    run {
            ...
            for (i = 0; i < raw.n_traits; i++) {
                sprintf(ps, "*%-9s", raw.trait_name[i]);
                fpr(fp);
                ...
                for (j = 0; j < raw.n_indivs; j++) {
                    if (j % 5 == 0 && j != 0)
                        fprint(fp, "\n          ");
                    sprintf(ps, "%12.3lf ", raw.trait[j][i]);
                    fpr(fp);
                }
                fnl(fp);
            }
            ...
            for (i = 0; i < raw.n_chroms; i++) {
                ...
                sprintf(ps, "chr%d %d\n", i + 1, raw.chrom_n_loci[i]);
                fpr(fp);
                for (j = 0; j < raw.chrom_n_loci[i]; j++) {
                    if (j % 18 == 0 && j != 0)
                        fprint(fp, "\n");
                    sprintf(ps, "%d ", order[loci_tot]);
                    fpr(fp);
                    loci_tot++;
                }
                fnl(fp);
                for (j = 0; j < raw.chrom_n_loci[i] - 1; j++) {
                    if (j % 12 == 0 && j != 0)
                        fprint(fp, "\n");
                    sprintf(ps, "%.4lf ", map_space[map_tot]);
                    fpr(fp);
                    map_tot++;
                }
                fnl(fp);
        ...
        for (i = 0; i < t_loc; i++) {
            if (t == order[i]) {
                ...
                for (k = 0; k < indivs; k++) {
                    ...
                    if (!parse_char(&ln, geno_chars, TRUE, &c)) { send(BADDATA); }
                    else if (c == 'B' && raw.data_type == BACKCROSS) { raw.locus[k][t] = 'H'; }
                    else { raw.locus[k][t] = c; }
                }
...
/* could send IOERROR or BADDATA */
    ...
    strcpy(raw.trait_name[num], &tok[1]);
    raw.trait_eqn[num][0] = '\0';
    ...
    for (i = 0; i < indivs; i++) {
        ...
        if (!rtoken(&ln, rREQUIRED, &raw.trait[i][num])) {
            if (!stoken(&ln, sREQUIRED, tok) || strcmp(tok, "-")) { send(BADDATA); }
...
void crunch_data(void) {
    ...
    for (i = 0; i < raw.n_indivs; i++) {
        ...
        for_locus_genotypes(raw.data_type, geno) raw.right_cond_prob[i][last][geno] =
    ...
    for (i = 0; i < raw.n_indivs; i++)
        for (k = first; k < last - 1; k++) {
            ...
            for_interval_genotypes(raw.data_type, geno) l[right_genotype[geno]] += indiv_probs[0][geno];

            indiv_interval_probs(indiv_probs, 1, i, k + 1, k + 2, raw.map_dist[k + 1]);
            for_interval_genotypes(raw.data_type, geno) r[left_genotype[geno]] += indiv_probs[1][geno];

            for_locus_genotypes(raw.data_type, geno) {
                if (fabs(l[geno] - r[geno]) >= 0.01) {
                    sprintf(ps, PROBS_NOT_EQ, i + 1, k + 2, geno);
                    pr();
                }
                left += l[geno];
                right += r[geno];
            }
            if (fabs(left - 1.0) >= 0.01) {
                sprintf(ps, PROBS_NOT_1, i + 1, k + 1);
                pr();
            }
            if (fabs(right - 1.0) >= 0.01) {
                sprintf(ps, PROBS_NOT_1, i + 1, k + 2);
                pr();
            }
    ...
    if (raw.data_type == BACKCROSS)
        ...
        switch (this_genotype) {
            ...
            case 'H':
                ...
                break;
            case 'C':
                prob[locus][H] = 2.0 / 3.0;
                prob[locus][B] = 1.0 / 3.0;
                break;
            case 'D':
                prob[locus][H] = 2.0 / 3.0;
                prob[locus][A] = 1.0 / 3.0;
                break;
            case '-':
                prob[locus][A] = 0.25;
                prob[locus][B] = 0.25;
        ...
        switch (this_genotype) {
            ...
            case 'D':
                ...
                break;
            case '-':
                prob[locus][A] = 3.0 / 8.0;
                prob[locus][B] = 3.0 / 8.0;
                prob[locus][H] = 0.25;
                break;
            default:
                send(CRASH);
...
real pheno_given_geno(char observation /* eg the 'phenotype' */, int genotype) {
    if (raw.data_type == BACKCROSS)
        switch (observation) {
            case 'A':
                return ((genotype == A) ? 1.0 : 0.0);
            case 'B':
            case 'H':
                return ((genotype == H) ? 1.0 : 0.0);
            case '-':
                return (1.0);
            default:
                send(CRASH);
...
void condition(LOCUS_GENOTYPE_PROBS *prob, int locus, int prev_locus, char observation, real rec_frac, int side) {
    ...
    for_locus_genotypes(raw.data_type, geno_is)total += prob[locus][geno_is] *= pheno_given_geno(observation, geno_is);
