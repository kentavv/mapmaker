...
void print_qtl_map(/* has CONT_VAR hooks */ QTL_MAP *map, bool *free_genetics) {
    int i, print_genetics, df_per;

    if (free_genetics == NULL) print_genetics = TRUE;
    else
        for (i = 0, print_genetics = FALSE; i < map->num_intervals; i++)
            if (!free_genetics[i]) {
                print_genetics = TRUE;
                break;
            }
    ...
    map_printer(map, print_genetics);
    nl();
    df_per = (raw.data_type == INTERCROSS ? 2 : 1);
    sprintf(ps, LIKE_LINE, map->chi_sq, df_per * map->num_intervals, map->log_like);
    pr();
...
void map_printer(QTL_MAP *map, bool print_genetics) {
    ...
    for (i = 0; i < map->num_intervals; i++) {
        sprintf(ps, INT_LINE, interval_str(map->left[i], map->right[i], TRUE),
                ...
                dist_str(map->qtl_pos[i], FALSE));
        pr();
        if (print_genetics) {
            sprintf(ps, GENETICS_FORMAT, genetics_str(&map->constraint[i], FALSE));
            pr();
        }
        sprintf(ps, WEIGHT_FORMAT, rsn(7.5, map->qtl_weight[i]));
        pr();
        if (raw.data_type == INTERCROSS) {
            sprintf(ps, DOM_FORMAT, rsn(7.5, map->qtl_dominance[i]));
            pr();
        }
        nl();
    }
    ...
    for (i = 0; i < map->num_continuous_vars; i++) {
        if (map->cont_var[i] == EPISTASIS_TERM) strcpy(trait_string, "epistasis");
            ...
            sprintf(trait_string, "%d (%s)", map->cont_var[i] + 1,
                    raw.trait_name[map->cont_var[i]]);

        sprintf(ps, CV_LINE, trait_string, rsn(7.5, map->cont_var_weight[i]));
        pr();
    }
}
...
void print_short_title(void) {
    /* FROB */
    sprintf(ps, INT_TITLE_LINE, " ", print_names ? "            " : "");
    pr();
    print(WEIGHT_TITLE);
    if (raw.data_type == INTERCROSS) print(DOM_TITLE);
    print(LOD_TITLE);
...
void print_short_qtl_map(/* FROB */ QTL_MAP *map, real threshold, real scale) {
    int i, n, last;
    ...
    sprintf(ps, INT_LINE, interval_str(map->left[last], map->right[last], TRUE),
            ...
            dist_str(map->qtl_pos[last], FALSE));
    pr();
    sprintf(ps, WEIGHT_FORMAT, rsn(7.5, map->qtl_weight[last]));
    pr();
    if (raw.data_type == INTERCROSS) {
        sprintf(ps, DOM_FORMAT, rsn(7.5, map->qtl_dominance[last]));
        pr();
    }
    sprintf(ps, LOD_FORMAT, map->var_explained * 100.0, map->log_like);
    pr();
...
do_print_E_step(real **expected_genotype, real **S_matrix, GENO_PROBS *expected_recs, int n_individuals, int n_genotype_vars, int n_continuous_vars,
    ...
    for (i = 0; i < n_individuals; i++) {
        ...
        pr();
        for (n = 0; n < num + 1; n++) {
            sprintf(ps, "%-6.3lf ", expected_genotype[i][n]);
            pr();
        }
        nl();
    }
    ...
    for (j = 0; j < n_intervals; j++) {
        ...
        for_locus_genotypes(raw.data_type, q) {
            ...
            nl();
            for_interval_genotypes(raw.data_type, g) {
                sprintf(ps, "%7.5lf ", expected_recs[j][g][q]);
                pr();
            }
            nl();
        }
...
void print_wiggle_genetics(GENETICS *genetics) {
    if (constrained(genetics)) {
        ...
        print(genetics_str(genetics, TRUE));
        nl();
...
void print_saved_wiggles(void) {
    ...
    sprintf(ps, WIG_LIST_TITLE, (print_names ? "     " : ""));
    pr();
    for (i = first_wiggle; i < num_wiggles; i++) {
        ...
        if (op->data == NULL)
            ...
            sprintf(ps, WIG_LIST_NUMS, i + 1, (op->num_orders == 1 ? "1" : "x"),
...
void print_peak(WIGGLE_PEAK *peak, int num) {
    ...
    print_map_divider();
    sprintf(ps, PEAK_TITLE, num + 1);
    pr();
    print(PEAK_L_CONF);
    if (peak->backward_pos == OFF_END) {
        i = interval_str(peak->backward_left, peak->backward_right, FALSE);
        sprintf(ps, PEAK_NOBOUND, i);
        pr();
    } else {
        i = interval_str(peak->backward_left, peak->backward_right, FALSE);
        sprintf(ps, PEAK_BOUND, i, dist_str(peak->backward_pos, FALSE));
        pr();
    }
    print(PEAK_R_CONF);
    if (peak->forward_pos == OFF_END) {
        ...
        i = interval_str(peak->forward_left, peak->forward_right, FALSE);
        sprintf(ps, PEAK_BOUND, i, dist_str(peak->forward_pos, FALSE));
        pr();
    }
    nl();
...
void print_test_wiggle_interval(QTL_MAP *map) {
    ...
    sprintf(ps, TEST_WIGGLE_INTERVAL1,
            ...
            dist_str(map->interval_len[last], FALSE), units_str(FALSE));
    pr();
...
void print_test_wiggle_title(void) {
    print(TEST_WIGGLE_DIVIDER);
    nl();
    print(TEST_WIGGLE_TITLE);
    nl();
    print(TEST_WIGGLE_DIVIDER);
    nl();
    print(TEST_WIGGLE_HEADER1);
    nl();
    print(TEST_WIGGLE_HEADER2);
    nl();
}
...
char *interval_str(int left, int right, bool fill) {
    ...
    if (print_mapm_loci) {
        if (!print_names)
            sprintf(str, "%d-%d", raw.original_locus[left],
                    raw.original_locus[right]);
        else
            sprintf(str, "%s-%s", raw.locus_name[left],
...
char *dist_str(/* Always takes 5 spaces */ real rec_frac, bool fill /* unused? */) {
    char *str;
    str = get_temp_string();
...
char *units_str(/* Takes 5 spaces, if fill, 2 otherwise */ bool fill) {
    char *str;
    str = get_temp_string();
        ...
        switch (genetics->interx_type) {
            ...
            case CONSTRAINED:
                if (!verbose) strcpy(str, "constrain");
                    ...
                    sprintf(str, "constraints: A=%-4.2lf B=%-4.2lf C=%-4.2lf",
                            genetics->a, genetics->b, genetics->c);
                break;
            case FIXED:
                if (!verbose) strcpy(str, "fixed");
                else
                    sprintf(str, "fixed: A=%-5.3lf D=%-5.3lf", genetics->a,
                            genetics->b);
                break;
            default:
                send(CRASH);
...
void print_tiny_map(/* unused, and broken */ QTL_MAP *map, int num, /* number for map (starts at 0) or num_intervals if map is null */ real offset) {
    ...
    if (num < 0) { /* header only */
        sprintf(ps, TINY_TITLE1, n_ints > 1 ? "S" : " ");
        pr();
        to_column(right);
        sprintf(ps, TINY_TITLE2, (offset > 0.0 ? "LIKE-DIFF" : "LOG-LIKE"));
        pr();

    } else { /* print the map */
        sprintf(ps, TINY_LINE1, num + 1);
        pr();
        for (i = 0; i < map->num_intervals; i++) {
        ...
        to_column(right);
        sprintf(ps, TINY_LINE2, map->var_explained * 100.0, map->log_like - offset);
        pr();
    }
}
...
void print_saved_compares(void) {
    ...
    sprintf(ps, COMP_LIST_TITLE, (print_names ? "     " : ""));
    pr();
...
void print_best_saved_maps(int compare, int contig, real threshold, real falloff) {
    ...
    run {
            ...
            for (i = 0, start = 0; i < contig; i++) { while (op->data[start]->contig) start++; }
...
/* while (op->data[start]->contig) {  */
            ...
            if (best >= threshold) {
                sprintf(ps, GOOD_MAPS_TITLE, (falloff < 0.0 ? "threshold minus " : ""));
                pr();
                print_map_divider();
            ...
            for (i = 0; i < num_in_list; i++) {
                ...
                if (good_maps_printed && ((falloff >= 0.0 && map->log_like > falloff) ||
                    ...
                    sprintf(ps, COMP_MAP_TOP, i + 1, map->log_like - best);
                    pr();
                    nl();
...
void print_saved_maps(int compare, int contig) {
    ...
    sprintf(ps, SAVED_BEST_LIKE, best);
    pr();
    nl();
...
void get_fixed_qtl_weights(QTL_MAP *map) {
    ...
    for (i = 0; i < map->num_intervals; i++) {
        ...
        if (raw.data_type == INTERCROSS &&
            !rtoken(&ln, rREQUIRED, &map->qtl_dominance[i]))
            error("bad weight!");
    }
