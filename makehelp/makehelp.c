...
void parse_entry(int kind, char *name, char *abbrev) {
    ...
    if (kind == TOP) {
        ...
        topic++;
        strcpy(section[topic], description);
        position[topic] = pos;
    } else {
...
void man_new_page(void) {
    ...
    sprintf(p, PAGE_NUM, abs(page));
    if (page > 0) page++; else page--;
    ps_page++;
...
void man_write_contents(void) {
    ...
    lines -= 2;
    strcpy(chapter_title, "Command Reference");
    page = -2;
    for (i = 0, s = 1; i < entries; i++) {
        switch (entry_type[i]) {
            case TOP:
                man_write_line("");
                strcpy(upcase, section[s]);
                uppercase(upcase);
                sprintf(temp, " %s\\(%d\\) %s ", (s >= 10 ? "" : " "), s, upcase);
                s++;
                break;
            case CMD:
                strcpy(upcase, entry[i]);
                uppercase(upcase);
                sprintf(temp, "      %s Command ", upcase);
                break;
            case OPT:
                strcpy(upcase, entry[i]);
                uppercase(upcase);
                sprintf(temp, "      %s Option ", upcase);
                break;
            case PAR:
                strcpy(upcase, entry[i]);
                uppercase(upcase);
                sprintf(temp, "      %s Parameter ", upcase);
                break;
            case HLP:
                strcpy(upcase, entry[i]);
                uppercase(upcase);
                sprintf(temp, "      %s Information ", upcase);
                break;
        }
