...
#ifdef DEBUGGING    
static void test_dump(real ***p, int rows, const char *name);
static void test_dump0(real **p, const char *name);
#endif
...
#ifdef DEBUGGING
	/* sprintf(ps,"iter: %2d  like=%f  thetas: %f %f\n",iter,new_like,
...
#define ss sufficient_stats
    ...
    if (ctm_state->error_kludge)
      count_error_lods(ctm_state, map);
/*    if (n_loci==2) {
	map->rec_frac[0][0]= 0.499999; 
	(*hmm_set_probs)(map,trans_prob,obs_prob,implied_recs);
	ctm_state->null_like= hmm_e_step(map,trans_prob,obs_prob,exp_transitions,
				  exp_observations);
    } */
#endif
...
static void setup_hmm(ctm_state_t *ctm_state, MAP *map)
{
    ...
    switch(type) {
    case F2_INTERCROSS: setup_f2_hmm(ctm_state, map->locus/*, map->error_rate*/);            break;
    case F3_SELF:       setup_f3_self_hmm(ctm_state, map->locus);                        break;
    case F2_BACKCROSS:
    case RI_SELF:
    case RI_SIB:        setup_bc_like_hmm(ctm_state, map->locus/*, map->error_rate*/, type); break;
...
static void
setup_bc_like_hmm(ctm_state_t *ctm_state,
		  int *locus /* The locus numbers in order */,
		  /*double *error_rate,*/
		  int cross_type)
{
    ...
    switch(cross_type) {
    case F2_BACKCROSS: ctm_state->ri_type=RI_NOT;  break;
    case RI_SELF:      ctm_state->ri_type=RI_SELF; break;
    case RI_SIB:       ctm_state->ri_type=RI_SIB;  break;
    default:           send(CRASH);                break;
    }
	...
	recs= ctm_state->implied_recs[i]; norecs= ctm_state->implied_norecs[i];
...
static void setup_f2_hmm(ctm_state_t *ctm_state, int *locus /* The locus numbers in order */  /*, double *error_rate*/)
{
    ...
    for (j=0; j<ctm_state->n_indivs; j++) {
	obs= ctm_state->observation[j][0]; total=0.0;
...
static void setup_f3_self_hmm(ctm_state_t *ctm_state, int *locus /* The locus numbers in the sequence */)
{
    ...
    */
    ...
    ctm_state->max_states=16; ctm_state->f3_states=0;	 
    ctm_state->max_observations=6; /* OBS_A, _B, _H, _NOT_A, _NOT_B, or _MISSING */
    ctm_state->separate_recs=FALSE;
	...
	for (j=0; j<ctm_state->n_indivs; j++) {
		...
		case PARENTAL_TYPE_A:	ctm_state->observation[j][i]=OBS_A; break;
		case PARENTAL_TYPE_B:	ctm_state->observation[j][i]=OBS_B; break;
		case HYBRID_TYPE_H:	ctm_state->observation[j][i]=OBS_H; break;
		case TYPE_NOT_A:	ctm_state->observation[j][i]=OBS_NOT_A; break;
		case TYPE_NOT_B:	ctm_state->observation[j][i]=OBS_NOT_B; break;
		case MISSING_DATA:	ctm_state->observation[j][i]=OBS_MISSING; break;
		default:		send(CRASH);
	    }
	    /*
	    if (i<2) { 
...
/* for now, assume obs_probs are fixed and initialized by setup_hmm() */
{
	...
	prob=ctm_state->trans_prob[i]; norec=1.0-rec;
...
static void test_dump(real ***p, int rows, const char *name) 
{
	...
	for (i=0; i<rows; i++) {
	    printf("%2d: ",i); 
	    for (j=0; j<max_states; j++) { printf("%10.8f ",p[k][i][j]);  }
	    printf("\n");
	}
    }
}
...
static void test_dump0(real **p, const char *name) 
{
    ...
    for (k=0; k<n; k++) {
	printf("%2d: ",k); 
	for (j=0; j<max_states; j++) { printf("%10.8f ",p[k][j]);  }
	printf("\n");
    }
}
#endif
...
#define state_not_allowed(locus,obs,state) \
  (obs!=OBS_MISSING && ctm_state->obs_prob[locus][obs][state]<0.24)
...
static void count_error_lods(const ctm_state_t *ctm_state, MAP *map)
{
	...
	map->error_lod[0][j]= map->error_lod[ctm_state->n_loci-1][j]= 0.0;
	for (i=1; i<ctm_state->n_intervals; i++) {
	    map->error_lod[i][j]= 0.0;
	    if (map->error_rate[i]<VERY_SMALL) continue;
	    apostiori_rate= 0.0;
	    the_obs= ctm_state->observation[j][i];
	    for (state=0; state<ctm_state->max_states; state++) {
		if (state_not_allowed(i,the_obs,state)) {
		    apostiori_rate+= ctm_state->exp_genotype[j][i][state];
		}
	    }
	    e1= map->error_rate[i]; e2= apostiori_rate;
	    like= (e2/e1) * ((1.0-e1)/(1.0-e2));
	    if (like>VERY_SMALL) map->error_lod[i][j]= log10(like);
	      else map->error_lod[i][j]= 0.0;
	}
    }
}
#endif
...
#if 0
static void (*hmm_set_probs)();
static bool error_kludge; /* TRUE for ERROR-KLUDGE, e.g. error_rate>0 */
static bool separate_recs; 
/* TRUE if distinct implied_rec/norec entries for each interval */
#endif
...
static SUFF_STATS **sufficient_stats; /* [locus] => ptr to struct */
#endif

#if 0
/* Internals for HMM E-Step (obs stuff moved to toplevel,h) */
static real **left_prob, **right_prob;  /* [locus#][state#] */
static real ***indiv_transitions;       /* [interval][from_state][to_state] */
static real ***indiv_observations;      /* [locus][observation][state], maybe NULL */
#endif

#if 0
/**** statics all setup by setup_hmm() - arrays are freed by free_hmm() ****/
static int n_indivs, n_loci, n_intervals;
static int max_observations, max_states;
static int  **n_poss_states, ***poss_state;     /* [indiv][locus],[indiv][locus][#] */
static real ***trans_prob, ***exp_transitions;  /* [interval][from_state][to_state] */
static real ***implied_recs, ***implied_norecs; /* [interval][from_state][to_state] */
static real ***obs_prob, ***exp_observations;   /* [locus][observation/MD][state] */
static bool *obs_prob_fixed;  /* [locus] - if all are fixed then exp_obs==NULL */
static int  **observation;    /* [indiv][locus] => OBS_ code */
static int  *the_observation; /* [indiv] a temp used only in f2_setup_hmm */
static real **apriori_prob;   /* [indiv][state] - for leftmost locus */
static int  no_data;          /* index [indiv] into apriori_prob for right locus */
static real *e_step_temp;     /* [state]- used in LCP and RCP calculations */
static real ***exp_genotype;  /* [indiv][locus][state] for ERROR-KLUDGE */
#endif

#if 0
static int ri_type;  /* RI_SELF, RI_SIB, or RI_NOT */
#endif
...
#if 0
/* There are 4 F2 states and 16 hidden F2/F3 states: */
static int A_AMAM, A_AMAP, A_APAM, A_APAP, H_AMAM, H_AMBP, H_BPAM, H_BPBP;
static int J_BMBM, J_BMAP, J_APBM, J_APAP, B_BMBM, B_BMBP, B_BPBM, B_BPBP;
/* These are temps for setup_f3_self_hmm(), although f2_recs is also 
   used in hmm_f3_self_set_probs() */
static int f3_states, mat_chrom[16], pat_chrom[16], f2_state[16];
static real f2_recs[4][4];
#endif
