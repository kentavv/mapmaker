...
void hmm_converge_to_map(MAP *map) {
    ...
    iter = 0;
    testing = 0;
    one = (n_loci >= 3 ? 1 : 0); /* just debugging vars */
    ...
    for (i = 0; i < n_loci; i++) {
        ...
        for (j = 0; j < n_indivs; j++) {
            ...
            switch (raw.data.f2.allele[locus[i]][j]) {
                case PARENTAL_TYPE_A:
                    observation[j][i] = OBS_A;
                    break;
                case PARENTAL_TYPE_B:
                    observation[j][i] = OBS_B;
                    break;
                case HYBRID_TYPE_H:
                    observation[j][i] = OBS_H;
                    break;
                case TYPE_NOT_A:
                    observation[j][i] = OBS_NOT_A;
                    break;
                case TYPE_NOT_B:
                    observation[j][i] = OBS_NOT_B;
                    break;
                case MISSING_DATA:
                    observation[j][i] = OBS_MISSING;
                    break;
                default:
                    send(CRASH);
            }
            if (i < 2) {
                if (observation[j][i] == OBS_A) {
                    a[i] += 1;
                    n[i] += 1;
                }
                if (observation[j][i] == OBS_B) {
                    b[i] += 1;
                    n[i] += 1;
                }
                if (observation[j][i] == OBS_H) {
                    h[i] += 1;
                    n[i] += 1;
                }
            }
...
"AA=%4d  AH=%4d  AB=%4d\nHA=%4d  HH=%4d  HB=%4d\nBA=%4d  BH=%4d  BB=%4d\n\n",
    ...
    for (i = 0; i < f3_states; i++)
        for (j = 0; j < f3_states; j++) {
