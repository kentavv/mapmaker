...
print_special_map(
    ...
    if (!map->sex_specific) { /**************** CEPH or F2 ****************/
        ...
        sprintf(ps, "  Markers          Distance ");
        pr(); /* that's 28 chars */
        if (map->error_lod != NULL) {
            sprintf(ps, "  Prob  Candidate Errors");
            pr();
        }
        nl();
        ...
        for (i = 0; i < map->num_loci - 1; ++i) {
            p1 = p2 = ' ';
            if (num_old > 0)
                for (p1 = '(', p2 = ')', j = 0; j < num_old; j++)
                    if (old_locus[j] == map->locus[i]) {
                        p1 = p2 = ' ';
                        break;
                    }
            ...
            if (map->error_lod != NULL && i > 0) { /* must be F2 data */
                ...
                if (!print_all_errors) { /* print the two worst and a count */
                    ...
                    if (n == 0) print("-");
                        ...
                        for (m = 0, j = 0; j < raw.data.f2.num_indivs; j++)
                            if ((lod = map->error_lod[i][j]) >= error_lod_thresh) {
        ...
        sprintf(ps,
                ...
                rsd(5.1, length), map->num_loci, map->log_like);
        pr();
        ...
        sprintf(ps, "    %s cM               %s cM\n\n",
                rsd(5.1, male_length), rsd(5.1, female_length));
        pr();
...
print_trys(
    ...
    sprintf(line2, empty_loc_str);
    sprintf(line1, empty_loc_str);
    strcat(line2, "   ");
    strcat(line1, "   ");
    ...
    for (i = 0; i < num_tried; i++) {
        if (list[i]->num_maps == 0) send(CRASH);
            ...
            map = get_best_map(list[i]);
            best[i] = map->log_like;
        }
        if (new_marker[i + first][1] != NO_LOCUS) { /* then paired */
            last = 1;
            while (new_marker[i + first][last + 1] != NO_LOCUS) last++;
    ...
    for (i = 0; i <= base_map->num_loci; i++) { /* i=interval */
        ...
        if (i < base_map->num_loci) {
            print(loc2str(base_map->locus[i]));
            print(" |");
    ...
    space(len(empty_loc_str));
    print(" |");
    for (i = 0; i < num_tried * width_ea - 1; i++) print("-");
    print("|\n");

    print("INF");
    space(len(empty_loc_str) - 3);
    print(" |");
    ...
    for (i = 0; i < num_loci; i++) {
        if (haplotyped(locus[i])) {
            ...
            sprintf(ps, " %4d  %-9s = [", haplo_first[locus[i]] + 1,
                    locname(haplo_first[locus[i]], TRUE));
            pr();
...
print_locus_summary(int *locus, int n_loci, bool haplo) {
    ...
    for (i = 0; i < n_loci; i++) {
        ...
        sprintf(ps, LIST_LINE, locus[i] + 1, locname(locus[i], haplo),
                ...
                error_rate[locus[i]] * 100.0, chrom, lg, hap);
        pr();
...
print_f2_map_genotypes(
    ...
    do { /* loop over sets of indivs */
        ...
        print("                  ");
        for (indiv = firsti; indiv <= lasti; indiv++) {
            sprintf(ps, "%d", ((indiv + 1) / 10) % 10);
            pr();
        }
        print("\n                  ");
        for (indiv = firsti; indiv <= lasti; indiv++) {
            sprintf(ps, "%d", (indiv + 1) % 10);
            pr();
        }
        print("\n                  ");
        ...
        for (i = 0; i < map->num_loci; i++) {
            ...
            if (num_old > 0)
                for (old = FALSE, j = 0; j < num_old; j++)
                    if (old_locus[j] == map->locus[i]) {
                        old = TRUE;
                        break;
                    }
            if (map->error_rate != NULL && i != 0 && i < map->num_loci - 1)
                error_rate = map->error_rate[i];
            else error_rate = NOER;
        ...
        for (indiv = firsti; indiv <= lasti; indiv++)
            if (num_recs[indiv] > 9) print("*");
                ...
                sprintf(ps, "%d", num_recs[indiv] % 10);
                pr();
            }
        nl();
        nl();
