...
/**** order maker ****/
    ...
    npt_min_indivs = 1;
    npt_codominant = FALSE;
    npt_min_theta = ONE_CM;
...
/**** F2 Stuff, including error checker ****/
    ...
    error_single_thresh = 2.0;
    error_net_thresh = 3.0;
...
set_default_linkage(void) {
    ...
    sprintf(ps, DEF_LINK_LOD, default_lod);
    pr();
    if (units != RECFRACS)
        sprintf(ps, DEF_LINK_THETA, "centimorgan distance",
...
set_3pt_linkage(void) {
    ...
    sprintf(ps, DEF_3PT_LOD, triplet_lod);
    pr();
    if (units != RECFRACS)
        sprintf(ps, DEF_3PT_THETA, "centimorgan distance",
                ((*mapfunction->rec_to_dist)(triplet_theta)) * 100.0);
    else sprintf(ps, DEF_3PT_THETA, "recombination-fraction", triplet_theta);
    pr();
    sprintf(ps, DEF_3PT_NUM, triplet_num_links);
    pr();
}
...
set_3pt_threshold(void) {
    ...
    sprintf(ps, TRIP_THRESH, three_pt_threshold, three_pt_window);
    pr();
}
