...
command draw_map(void) {
    ...
    run {
            if (!make_filename(name, FORCE_EXTENSION, PS_EXT)) {
                sprintf(ps, "illegal filename '%s'", name);
                error(ps);
            }
        ...
        if (msg == CANTOPEN) {
            sprintf(ps, "Can't create output file '%s'", name);
            error(ps);
    ...
    run {
            ...
            if (threshold != VERY_UNLIKELY) {/* can you say "abstraction violation" */
                while (i < list->num_maps &&
                       list->map_list[i]->log_like > (best + threshold))
                    i++;
                num_to_print = i;
                sprintf(ps, BEST_THRESH, maybe_s(num_to_print), -threshold);
                pr();
            } else {
            ...
            if (excluded > 0) {
                sprintf(ps, N_EXCLUDED, excluded);
                pr();
            }
            print(":\n");
            ...
            nl();
            unorder_first[0] = NO_LOCUS;
            order_next[prev] = NO_LOCUS;
            num_orders = 1;
...
command ripple(void) {
    ...
    if (!irange(&window, 3, 99)) usage_error(1);
    get_arg(rtoken, 2.0, &thresh);
    if (!rrange(&thresh, 0.0, 100.0)) usage_error(1);
    if (window > n_loci)
    ...
    run {
            ...
            for (i = 0; i <= n_loci - window; i++) {
                ...
                while (k < list->num_maps && /* thresh>0 here */
                       list->map_list[k]->log_like > (best - thresh))
                    k++;

                if (!same || k > 1) {
                    sprintf(ps, "\nbest order%s:", maybe_s(k));
                    pr();
                    if (excluded > 0) {
                        sprintf(ps, " (%d excluded)", excluded);
                        pr();
                    }
                    nl();
                    print_list(list, k);
            ...
            while (stoken(&markers, sREQUIRED, token)) {
                if (streq(token, "[")) {
                    ...
                    if (!is_a_locus(token, &n, &err) || !nullstr(err)) {
                        sprintf(ps, "'%s' is not a locus", token);
                        error(ps);
                    }
                ...
                for (first_marker = 0; first_marker < num_tries;
                    ...
                    for (i = 0, j = first_marker; j < num_tries && i < num_to_try_at_once;
                        ...
                        if (!try_marker(new_marker[j], map, list[i],
                            ...
                            sprintf(ps, TRY_NONE, rag(loc2str(marker_to_try[j])));
                            pr();
                        }
                    }
                    nl();
...
command order_maker(void) {
    ...
    if (!rtoken(&args, default_theta, &thetabound) || !input_dist(&thetabound))
        usage_error(2);
    get_arg(itoken, 5, &seed_size);
    if (seed_size < 3) usage_error(0);
    get_arg(rtoken, 3.0, &seed_like);
    if (seed_like <= 0.0) usage_error(0);
    get_arg(itoken, 50, &seed_tries);
    if (seed_tries < 1) usage_error(0);
    nomore_args(0);
    seed_like = -seed_like;

    sprintf(ps, LINK_CRITERIA, lodbound, rag(rf2str(thetabound)));
    pr();
    sprintf(ps, START_CRITERIA, seed_size, -seed_like, seed_tries);
    pr();
            ...
            do {
                ...
                if (group_size >= 2) {
                    ...
                    if (group_size < 3) {
                        ...
                        pr();
                        for (i = 0; i < group_size; i++) {
                            sprintf(ps, "%s ", rag(loc2str(linkage_group[i])));
                            pr();
                    ...
                    if (num_unplaced > 0) {
                        ...
                        nl();
                        sprintf(ps, ORDER_AT, npt_first_threshold);
                        pr();
                        ...
                        if (npt_first_threshold != npt_threshold && num_unplaced > 0) {
                            print(SUB_DIVIDER);
                            sprintf(ps, ORDER_AT, npt_threshold);
                            pr();
                            extend_order(order, unplaced, &num_unplaced,
                                         -npt_threshold, FALSE);
                        }
                    ...
                    for (i = 0; i < order->num_loci; i++) {
                        sprintf(ps, "%s ", rag(loc2str(order->locus[i])));
                        pr();
                    ...
                    nl();
                    sprintf(ps, "other%d= ", groups_done);
                    pr();
                    for (i = 0; i < num_unplaced; i++) {
                        sprintf(ps, "%s ", rag(loc2str(unplaced[i]->locus)));
                        pr();
