...
isa_chrom(
    ...
    *chrom = NO_CHROM;
    ...
    for (i = 0; i < chromosome->num_maps; i++)
        if (xstreq(name, chromosome->map_list[i]->map_name)) {
            *chrom = i;
            break;
        }
   ...
   assume haplo_sanity is true for old framework, and force it for new one. */
{
    ...
    for (j = 0; j < new->num_loci; j++) {
        ...
        if (!assigned(new->locus[j])) {
            sprintf(ps, FRAME_NO_CHROM, loc2str(new->locus[j]));
            error(ps);
...
get_chrom_loci(int chrom, int *locus, int which_loci, int *num_loci, int *num_framework) {
    ...
    if (which_loci >= HAPLOS) {
        haplos_anyway = TRUE;
        which_loci -= HAPLOS;
    }
    if (which_loci >= NON_FRAME) {
        non_frame = TRUE;
        which_loci -= NON_FRAME;
    }
    if (which_loci >= FRAMEWORK) {
        framework = TRUE;
        which_loci -= FRAMEWORK;
    }
    ...
    if (non_frame)
        for (i = 0; i < raw.num_markers; i++) {
            if (use_haplotypes && !haplos_anyway && haplotyped(i) &&
                haplo_first[i] != i)
                continue; /* then skip non-primaries */
...
is_assignable(
    ...
    if (anchor_locus(locus) && old != chrom) {
        sprintf(ps, ASS_ANCHOR, loc2str(locus), chrom2str(old));
        pr();
    ...
    if (framework_marker(locus) && old != chrom) {
        ...
        if (!fix_frames) {
            ...
            sprintf(ps, ASS_ISFRAME, loc2str(locus), chrom2str(old));
            pr();
            return (FALSE); /* ??? */
        }
    }
...
set_chrom_anchors(
    ...
    for (j = 0; j < num_loci; j++) {
        ...
        sprintf(ps, ANCHOR_ISNOW, loc2str(locus[j]), chrom2str(chrom));
        pr();
...
do_assignments(
    ...
    run {
            ...
            array(count, num_chroms, int);
            for (i = 0; i < num_chroms; i++) count[i] = 0;
            if (!get_anchors(anchor, count, locus, num_loci, haplo))
                error(DOASS_NOANCHS);

            lod = lod1;
            unlinked_lod = unlinked_lod1;
            theta = theta1;
            assigned_any = TRUE;
    ...
    for (j = 0; j < num_groups; j++) {
        ...
        if (this_group) { /* matched one in this group */
            if (pos_chrom == NO_CHROM ||
                ...
                pos_chrom = this_chrom;
                pos_locus = assigned_locus;
                pos_group = j;
                pos_lod = this_lod;
                pos_theta = this_theta;
...
is_placeable(int locus, int chrom) {
    ...
    if (!force_haplo_sanity(&locus, FALSE)) {
        ...
        sprintf(ps, PLACE_NOTASS, locus + 1, locname(locus, TRUE));
        pr();
        /* unplace_this(locus,NO_CHROM,M_UNKNOWN,FALSE); */
        return (FALSE);
        ...
        /* not used yet */
        sprintf(ps, PLACE_NOTCHR, locus + 1, locname(locus, TRUE), chrom2str(chrom));
        pr();
        /* unplace_this(locus,NO_CHROM,M_UNKNOWN,FALSE); */
        return (FALSE);
        ...
        if (!excluded[j]) {
            if (place[j]->like > PLACEMENT_THRESHOLD) {
                if (n < MAX_STORED_PLACES) {
                    ...
                    sprintf(ps, PLACE_TOOMANY, locus + 1, locname(locus, TRUE));
                    pr();
                    return (0);
                }
            }
...
second_best_placement( /* only ok for M_REGION */
    ...
    for (j = 0; j < placement[locus]->num_intervals; j++)
        if (j != i && placement[locus]->like_ratio[j] > best) { best = placement[locus]->like_ratio[j];; }
    ...
    for (i = 0; i < num_changed; i++) {
        ...
        if (assignment[n]->status == A_ANCHOR) {
            sprintf(ps, "warning: %s WAS an anchor locus for chromosome %s\n",
                    locname(n, FALSE), chrom2str(assignment[n]->chromosome));
            pr();
        }
    }
    for (i = 0; i < num_changed; i++) {
...
write_mapping_data(FILE *fp) {
    ...
    for (locus = 0; locus < raw.num_markers; locus++) {
        ...
        fpr(fp);
        if (assignment[locus]->status == M_UNKNOWN) {
            fnl(fp);
            continue;
        }
