...
void data_init(void) {
    ...
    array(BADDATA_text, MAXLINE + 1, char);
    array(BADDATA_reason, MAXLINE + 1, char);
...
char *data_info(bool add_nums) {
    ...
    switch (raw.data.f2.cross_type) {
        case F2_INTERCROSS:
            strcpy(str1, "F2 intercross data ");
            break;
        case F2_BACKCROSS:
            strcpy(str1, "F2 backcross data ");
            break;
        case RI_SIB:
            strcpy(str1, "RI (sib-mating) data ");
            break;
        case RI_SELF:
            strcpy(str1, "RI (selfing) data ");
            break;
        case F3_SELF:
            strcpy(str1, "F3 intercross (selfing) data");
            break;
        default:
            send(CRASH);
    }
    ...
    if (!add_nums) return (str1);
        ...
        sprintf(str2, "%s (%d individuals, %d loci)", str1,
...
FILE *start_save_to_file(char *name, char *ext, char *type, bool *exists) {
    ...
    *exists = FALSE;
    ...
    run {
            ...
            sprintf(ps, SAVING_TO, type, name);
            pr();
            flush();
        } except_when(CANTOPEN) {
        sprintf(ps, NOT_SAVING_TO, name);
        pr();
        nl();
        abort_command();
    }
...
void finish_save_to_file(char *name, char *oldext, bool exists) {
    ...
    make_filename(tmpname, FORCE_EXTENSION, TEMP_EXT);
    strcpy(oldname, name);
    make_filename(oldname, FORCE_EXTENSION, oldext);
...
void do_load_data(FILE *fp, char *filename, bool israw) {
    ...
    make_filename(name, FORCE_EXTENSION, MAPS_EXT);
    strcpy(type, "map");
    run {
            fp2 = NULL;
            fp2 = open_file(name, READ);
            if (read_magic_number(fp2, type)) {
                sprintf(ps, LOADING_FROM, type, name);
                pr();
                flush();
                read_order_data(fp2);
                ...
                sprintf(ps, NOT_LOADING_FROM, type, name);
                pr();
            }
            close_file(fp2);
        } except_when(CANTOPEN) {} /* need a better handler */

    make_filename(name, FORCE_EXTENSION, TWO_EXT);
    strcpy(type, "two-point");
    run {
            fp2 = NULL;
            fp2 = open_file(name, READ);
            if (read_magic_number(fp2, type)) {
    ...
    make_filename(name, FORCE_EXTENSION, THREE_EXT);
    strcpy(type, "three-point");
    run {
            fp2 = NULL;
            fp2 = open_file(name, READ);
            if (read_magic_number(fp2, type)) {
...
#endif
    strcpy(raw.filename, "");
    raw.data_type = NO_DATA; /* use cross_type below */
    undo_state();
...
read_raw_file_header(FILE *fp, char *filename, char *symbol) {
    ...
    if (!nullstr(ln)) { /* symbol definitions, old chrom#, case thing */
        ...
        while (!nullstr(ln) && sscanf(ln, "%s", str) == 1) {
            ...
            user[0] = str[0];
            if (uppercase_genotypes) uppercase(user);
    ...
    while (i < raw.data.f2.num_indivs) {
        ...
        if (!parse_char(&ln, symbol, SKIPWHITE, &c)) {
            if (c == '#') {
                ...
                sprintf(badstr, "illegal genotype character '%c' in locus %s",
...
void write_traits(FILE *fp) {
    ...
    for (i = 0; i < num_traits; i++) {
        sprintf(ps, "*%-10s", trait_name[i]);
        fpr(fp);
        for (j = 0; j < raw.data.f2.num_indivs; j++) {
            if (j % 5 == 0 && j != 0) fprintf(fp, "\n          ");
            sprintf(ps, "%12.3lf ", trait[i][j]);
            fpr(fp);
        }
        fnl(fp);
    }
