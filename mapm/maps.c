...
void init_for_ctm(/* what else */ MAP *map, bool sex, bool errors, bool start /* deal with start */) {
    ...
    if (sex && raw.data_type != CEPH) send(CRASH);
    map->allow_errors = errors;
    if (errors && raw.data_type != F2) send(CRASH);
    map->log_like = 0.0;
...
SAVED_LIST *allocate_map_list(int maxmaps, int maxloci, bool sortflag, MAP **map) {
    ...
    run {
        ...
        } except_when(ABORT) free_map_list(list);
    return (list);
}
...
void read_map(FILE *fp, MAP *map) {
    ...
    map->unlink = unlink;
    map->sex_specific = sex;
    strcpy(map->map_name, name + 1);
    map->allow_errors = errors;
    map->num_loci = num_loci;
    map->log_like = like;
...
void write_map(FILE *fp, MAP *map) {
    ...
    sprintf(ps, "*%s %d %d %d %d %.3lf\n",
            ...
            map->allow_errors, map->log_like);
    fpr(fp);

    for (i = 0; i < map->num_loci; i++) {
        if (i % 10 == 0 && i != 0 && i != map->num_loci - 1) fnl(fp);
        sprintf(ps, "%d ", map->locus[i]);
        fpr(fp);
    }
    fnl(fp);
    ...
    for (i = 0; i < map->num_loci - 1; i++) {
        ...
        sprintf(ps, "%6.4lf ", map->rec_frac[i][MALE]);
        fpr(fp);
    }
    fnl(fp);
    ...
    if (raw.data_type == CEPH && map->sex_specific) {
        for (i = 0; i < map->num_loci - 1; i++) {
            if (i % 10 == 0 && i != 0) fnl(fp);
            sprintf(ps, "%6.4lf ", map->rec_frac[i][FEMALE]);
            fpr(fp);
        }
        fnl(fp);
    }
    ...
    for (i = 0; i < map->num_loci - 1; i++) {
        ...
        sprintf(ps, "%d ", map->fix_interval[i]);
        fpr(fp);
    }
    fnl(fp);
