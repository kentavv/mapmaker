...
list_chroms(void) {
    ...
    sprintf(ps, CHROMS_FORMAT, "Total:", sum_total, sum_frame, sum_anchor,
            sum_placed, sum_unique, sum_region);
    pr();
    unarray(loci, int);
}
...
place(void) {
    ...
    run {
            ...
            sprintf(ps, PLACE_HEADER, npt_thresh);
            pr();
            ...
            for (chrom = 0; chrom < chromosome->num_maps; chrom++) {
                ...
                for (i = 0; i < n_loci; i++)
                    if (locus[i] != NO_LOCUS)
                        if (assigned_to(locus[i], chrom) && is_placeable(locus[i], chrom)) {
                            chrom_locus[n_to_place++] = locus[i];
                            locus[i] = NO_LOCUS;
                        }
                ...
                print_long_map(frame, "Framework Map:");
                nl();
...
show_chrom(void) {
    ...
    run {
            ...
            sprintf(ps, CHROM_MARKS, chrom2str(chrom));
            pr();
            nl();
            ...
            if (use_haplotypes) {
                nl();
                print_haplo_summary(marker, num_markers);
            }
            print(MAP_DIVIDER);
...
place_together(void) {
    ...
    run {
            ...
            for (chrom = 0; chrom < chromosome->num_maps; chrom++) {
                ...
                nl();
                sprintf(ps, "other%d= ", num_orders + 1);
                pr();
