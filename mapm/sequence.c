...
void compile_sequence(char *str);

void next_token(char **p_str);

void seqcomp(char **p_str, SEQ_NODE **first_node, int term_char);
...
bool parse_seq_chrom(char *str, int *chrom, char **rest);
...
void allocate_seq_stuff(int n_loci) {
    ...
    array(maybe_seq, MAX_SEQ_LEN + 99, char);
    maybe_seq[0] = '\0';
    array(new_seq, MAX_SEQ_LEN + 99, char);
    new_seq[0] = '\0'; /* +99 for chrom */
    array(the_seq, MAX_SEQ_LEN + 99, char);
    the_seq[0] = '\0';
...
void print_sequence(void) {
    ...
    sprintf(ps, "sequence #%d= ", the_seq_history_num);
    pr();
    if (current_chrom != NO_CHROM) {
        sprintf(ps, "%s: ", chrom2str(current_chrom));
        pr();
    }
...
void reset_seq(SEQ_NODE *p, bool reset_insert_pos) {
    ...
    switch (p->type) {
        case MEGALOCUS:
            reset_seq(p->node.megalocus.contents, TRUE);
            break;
        case INVERTABLE_SET:
            reset_seq(p->node.invertable_set.contents, TRUE);
            p->node.invertable_set.flip = FALSE;
            break;
        case UNORDERED_SET:
            reset_seq(p->node.unordered_set.contents, TRUE);
            break;
    }
...
void parse_locus_args(int **loci, int *num_loci /* both side-effected */) {
    ...
    copy = rest;
    n = 0;
    was_range = FALSE; /* count and check pass */
    ...
    *num_loci = n;
    ...
    while (stoken(&copy, sREQUIRED, name))
        if (streq(name, SURROGATE_DASH)) {
            stoken(&copy, sREQUIRED, name);
            is_a_locus(name, &next, &errmsg);
   ...
   Also now checks for all balanced punctuation. Signal BADSEQ if fails. */
    ...
    for (i = 0; str[i] != '\0'; i++)
        if (match_chr != '\0' && str[i] == match_chr) return (i);
            ...
            if ((j = delete_comments(RIGHT_BRACKET_char, str + i + 1)) < 0) mismatch_(i, LEFT_BRACKET_char) else i = j;
        } else if (str[i] == LEFT_ANGLE_char) {
            if ((j = delete_comments(RIGHT_ANGLE_char, str + i + 1)) < 0) mismatch_(i, LEFT_ANGLE_char) else i = j;
        } else if (str[i] == LEFT_SET_char) {
            if ((j = delete_comments(RIGHT_SET_char, str + i + 1)) < 0) mismatch_(i, LEFT_SET_char) else i = j;
        } else if (str[i] == LEFT_PAREN_char) {
            if ((j = matching(RIGHT_PAREN_char, str + i + 1)) < 0) mismatch_(i, LEFT_PAREN_char)
            else {
                strdel(str + i, j + 2);
                despace(str);
                continue; /* with i+1 */
   ...
   defined names- this should never happen */
    ...
    while (!nullstr(str)) {
        ...
        if (is_a_sequence(token, &expansion, &errmsg)) {
            ...
            if (seq_temp != expansion)
                strcpy(seq_temp,
                       expansion); /* if-statement to avoid valgrind warning of overlapping strings when pointers seq_temp = expansion, but why are they equal?) */
...
bool is_a_named_locus(char *str /* must be a single token, downcased */, int *n) {
    ...
    for (i = 0; i < raw.num_markers; i++)
        if (xstreq(str, raw.locus_name[i])) {
            *n = i;
            return (TRUE);
        }
    return (FALSE);
    ...
    *seq = *why_not = ptr_to("");
    ...
    for (i = 0; i < n; i++) {
        if (ordered_seq) {
            ...
            contig = 0;
            j = i;
            while (j != n - 1 && seq_locus[j + 1] == seq_locus[j] + 1) {
                j++;
                contig++;
            }
            if (contig < 2) {
                ...
                if (!print_names)
                    ...
                    sprintf(last, "%s-%s", raw.locus_name[seq_locus[i]],
...
void print_special_sequences(void) {
    ...
    for (i = 0; i < num_groups - 1; i++) /* groupN */
    {
        ...
        print_special_sequence(name, FALSE);
    }
    print_special_sequence("unlinked", FALSE);

    for (i = 0; i < num_orders; i++) { /* orderN */
        sprintf(name, "order%d", i + 1);
        print_special_sequence(name, TRUE);
        sprintf(name, "other%d", i + 1);
        print_special_sequence(name, FALSE);
    }
